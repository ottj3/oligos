...
class Acid:
    ...
    def compute_required_acids_with_scores(self, locations, minval, maxval, numpts):
        ...
        self.required_acids = [np.floor(x) for x in points_required]
        while sum(self.required_acids) < total_required:
            differences = [x-y for x,y in zip(points_required, self.required_acids)]
            index_of_max = differences.index(max(differences))
            self.required_acids[index_of_max]+=1
        self.required_acids = [int(x) for x in self.required_acids]
        return (self.required_acids, [score for nDeltas, score in self.designtype])

    def compute_acid_designs(self, locations, segsize, overlapsize, minval,
                             maxval, numpts, seqlength):
        required_acids, scores = self.compute_required_acids_with_scores(locations,minval,maxval,numpts)
        designlist = get_possible_intervals(seqlength, locations, required_acids[0], scores[0])
        for i in range(1,len(required_acids)):
            designlist = disjoint_combos(designlist,
                                         get_possible_intervals(seqlength, locations, required_acids[i], scores[i]))
        return designlist


def overlap_test(array1, array2):
    "Tests if two vectors have nonzero entries at any common index"
    if sum(array1*array2) != 0:
        return True
    else:
        return False
...
def compute_best_design(p, aoi, segsize, overlapsize, mins, maxs, numpts):
    ...
    # print "minscore: ", minscore, "\ncoverage array: ", scorearray, "\n\n"
    # if type(minindex) == int:
    #     print aoi[0], ":", acidlist[0].designlist[minindex].intervals, '\n', acidlist[0].designlist[minindex].score, '\n\n'
    # else:
    #     for i in range(len(minindex)):
    #         print aoi[i], ":", acidlist[i].designlist[minindex[i]].intervals, "\n", acidlist[i].designlist[minindex[i]].score, "\n\n"
    results = []
    if type(minindex) == int:
...
if __name__ == "__main__":
    ...
    # P='RNDDDDDDDNDRRNNDDDRDRRRRNDDRNRNNDDDDNDRRNDDDDDDNNNDDDDNDRRRDDDDNDRRRRRNNRRRNNDDDDDDRRNDDDDDD'
    # SEGSIZE=11
    # OVERLAPSIZE=2
    # AOI='RN'
    # MINS=[.2, .2]
    # MAXS=[.8, .7]
    # NUMPTS=[4, 6]
    # compute_best_design(P,AOI,SEGSIZE,OVERLAPSIZE,MINS,MAXS,NUMPTS)
    # # test comments
